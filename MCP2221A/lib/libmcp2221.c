...
{
    // The I2C slave address is represented on 8 bits
    // with even values with writes (last bit == 0)
    // and  off values for reads    (last bit ==1)
    // To get the 8-bit representation out of a 7-bit address, 
    // we left shift the 7-bit value by 1.
...
mcp2221_error LIB_EXPORT mcp2221_i2cRead(mcp2221_t* device, int address, int len, mcp2221_i2crw_t type)
{
    ...
    // with even values with writes (last bit == 0)
    // and  off values for reads    (last bit ==1)
    // To get the 8-bit representation out of a 7-bit address for read, 
    // we left shift the 7-bit value by 1, and add 1 to the shifted value

	address <<= 1;
    address += 1; // Add 1 to signal read
	...
	report[1] = len;
	report[2] = len>>8;
    // Original code: report[3] = address; The int (address) is implicitly converted to a uint8_t upon assignment
	report[3] = (uint8_t)address; // Modified here for clarity, explicit casting.
	res = doTransaction(device, report);
	return res;
}
